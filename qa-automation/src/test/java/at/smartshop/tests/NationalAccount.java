...
public class NationalAccount extends TestInfra {
	...
	public void verifNationalAccAddFunctionality() {
		try {
			...
			foundation.waitforElement(NationalAccounts.DPD_ORGANIZATION, 2);
			...
			navigationBar.navigateToMenuItem(rstNavigationMenuData.get(CNNavigationMenu.MENU_ITEM));
			foundation.waitforElement(NationalAccounts.TBL_BODY, 2);
	...
	public void verifGridColumnInNationalAccount() {
		try {
			...
			Assert.assertTrue(adminNationalAccounts
			...
			Assert.assertEquals(upcData.get(Constants.NATIONAL_CATEGORY),
			...
			Assert.assertEquals(NAData.get(Constants.UPCS),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));

			// getting rule price from UI
			table.selectRow(rstNationalAccountsData.get(CNNationalAccounts.RULE_NAME));
			double uiRulePrice = Double
					.parseDouble((foundation.getTextAttribute(AdminNationalAccounts.TXT_RULE_PRICE)));
			...
			Assert.assertEquals(rulePriceNoMoreThan.get(Constants.MIN_PRICE),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));
			Assert.assertEquals(rulePriceNoMoreThan.get(Constants.EXACT_PRICE),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));
			...
			Assert.assertEquals(rulePriceNoLessThan.get(Constants.EXACT_PRICE),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));
			...
			Assert.assertEquals(ruleForceExactPrice.get(Constants.MAX_PRICE),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));
			Assert.assertEquals(ruleForceExactPrice.get(Constants.MIN_PRICE),
					rstNationalAccountsData.get(CNNationalAccounts.MIN_MAX_EXACT_PRICE));
			...
			foundation.waitforElement(AdminNationalAccounts.BTN_CANCEL, 2);
			...
			foundation.waitforElement(AdminNationalAccounts.POP_UP_BTN_CANCEL, 2);
	...
	public void verifyDeleteButton() {
		try {
			...
			foundation.waitforElement(CreateNewRule.DELETE_PROMPT, 5);
			...
			foundation.waitforElement(AdminNationalAccounts.BTN_CREATE_NEW_RULE, 3);
	...
	public void MasterNationalAccountSummary() {
		try {
			...
			foundation.waitforElement(createNewRule.objClientRuleName(rulePageTitle), 4);
    ...
    public void verifyMNADeleteScreen() {
        try {
            ...
            foundation.waitforElement(NationalAccountRules.TBL_NATIONALACCOUNT_BODY, 2);
            ...
            foundation.waitforElement(nationalAccountRules.verifyPromptMsg(promptTitle.get(0)), 2);
	...
	public void NationalAccountSummary() {
		try {
			...
			foundation.waitforElement(NationalAccounts.BTN_POP_UP_YES, 2);
	...
	public void NationalAccountSummaryLocation() {
		try {
			...
			foundation.waitforElement(NationalAccounts.BTN_ADD_NATIONAL_ACCOUNT, 3);
			foundation.click(NationalAccounts.BTN_ADD_NATIONAL_ACCOUNT);
	...
	public void VerifyPopup() {
		try {
			...
			foundation.waitforElement(nationalAccounts.nationalAccountDetails(accountName), 2);
	...
	public void addRemoveLocationMasterAct() {
		try {
			...
			foundation.waitforElement(AdminNationalAccounts.DPD_ORG_MODAL, 2);
