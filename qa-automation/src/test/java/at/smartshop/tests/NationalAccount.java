...
//	}
	...
	public void verifyMenuOptionsInDashboardAndManageRulesLinkUsingMasterNA() {
		...
		try {
			...
			CustomisedAssert.assertTrue(
					foundation.getTextofListElement(AdminNationalAccounts.NAVIGATION_MENU).equals(requiredOptions));

			// navigate to Admin->National Account and verify manage roles in plural
			// functionality
			navigationBar.navigateToMenuItem(menu.get(0));
			CustomisedAssert.assertTrue(foundation.isDisplayed(AdminNationalAccounts.LBL_NATIONAL_ACCOUNTS_ARAMARK));
			CustomisedAssert
					.assertTrue(foundation.getText(AdminNationalAccounts.LNK_MANAGE_RULES).equals(client.get(2)));
	...
	public void verifyNASaveWithApostropheUsingSuperuser() {
		...
		try {
			...
			CustomisedAssert
					.assertTrue(foundation.getText(AdminNationalAccounts.NATIONAL_ACCOUNT_NAME).equals(data + "'s"));
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
		}
	}
