...
public class UserRoles extends TestInfra{
	...
	public void verifClientDropdownField() {
		try {
			...
			List<String> lblRowRecord = Arrays.asList(rstUserRolesData.get(CNUserRoles.ROW_RECORD).split(Constants.DELIMITER_TILD));
			...
			table.selectRow(lblRowRecord.get(0));			
 			...
 			List<String> clientdropDownList = Arrays.asList(rstUserRolesData.get(CNUserRoles.CLIENT_DROPDOWN).split(Constants.DELIMITER_TILD));
