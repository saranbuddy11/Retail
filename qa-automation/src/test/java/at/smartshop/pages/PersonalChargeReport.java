...
public class PersonalChargeReport extends Factory {
	...
	private static final By TBL_PERSONAL_CHARGE = By.id("rptdt");
	...
	public Map<Integer, Map<String, String>> getTblRecordsUI() {
		try {
			int recordCount = 0;
			tableHeaders.clear();
			WebElement tableReportsList = getDriver().findElement(TBL_PERSONAL_CHARGE_GRID);
			WebElement tableReports = getDriver().findElement(TBL_PERSONAL_CHARGE);
	...
	public void getRequiredRecord(String locationName, String transID) {
		try {
			boolean flag = false;
			for (int rowCount = 0; rowCount < intialData.size(); rowCount++) {
				if (intialData.get(rowCount).get(tableHeaders.get(0)).equals(locationName)
						&& intialData.get(rowCount).get(tableHeaders.get(7)).equals(transID)) {
		...
		try {
			...
			for (JsonElement item : items) {
				...
				if (reqString.equals(Reports.PAYMENTS)) {
					json.addProperty(Reports.TYPE, Reports.BADGE);
				}
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
