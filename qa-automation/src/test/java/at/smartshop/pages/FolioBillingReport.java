...
public class FolioBillingReport extends Factory {
	...
	private static final By TBL_FOLIO_BILLING = By.id("folioBillingRpt");
	private static final By LBL_REPORT_NAME = By
			.cssSelector("#report-container > div > div.col-12.comment-table-heading");
	private static final By TBL_FOLIO_BILLING_GRID = By.cssSelector("#folioBillingRpt > tbody");
	private static final By REPORT_GRID_FIRST_ROW = By.cssSelector("#folioBillingRpt > tbody > tr:nth-child(1)");
	...
	public Map<Integer, Map<String, String>> getTblRecordsUI() {
		try {
			int recordCount = 0;
			tableHeaders.clear();
			WebElement tableReportsList = getDriver().findElement(TBL_FOLIO_BILLING_GRID);
			WebElement tableReports = getDriver().findElement(TBL_FOLIO_BILLING);
			...
			for (int iter = 0; iter < scancodes.size(); iter++) {
				for (int val = 0; val < intialData.size(); val++) {
					if (intialData.get(val).get(tableHeaders.get(0)).equals(transDate)
							&& intialData.get(val).get(tableHeaders.get(3)).equals(scancodes.get(iter))) {
	...
	public void updateTotal() {
		try {
			for (int iter = 0; iter < requiredRecords.size(); iter++) {
				...
				String tax = taxData.get(iter);
				double updatedPrice = Double.parseDouble(price) + Double.parseDouble(deposit) + Double.parseDouble(tax)
	...
	private void getJsonArrayData() {
		try {
			...
			for (JsonElement item : items) {
				...
				taxData.add(element.get(Reports.TOTAL_TAX).getAsString());
		...
		try {
			...
			for (JsonElement item : items) {
				...
				if (reqString.equals(Reports.PAYMENTS)) {
					json.addProperty(Reports.TYPE, Reports.ROOM);
				}
			}
		} catch (Exception exc) {
			Assert.fail(exc.toString());
