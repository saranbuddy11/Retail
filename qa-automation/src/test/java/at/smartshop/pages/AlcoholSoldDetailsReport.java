...
public class AlcoholSoldDetailsReport extends Factory {
		...
		try {
			...
			for (int iter = 0; iter < scancodes.size(); iter++) {
				for (int val = 0; val < intialData.size(); val++) {
					if (intialData.get(val).get(tableHeaders.get(0)).equals(scancodes.get(iter))) {
	...
	public void updateSales(String columnName, String amount) {
		try {
			for (int iter = 0; iter < requiredRecords.size(); iter++) {
				String initialAmount = intialData.get(requiredRecords.get(iter)).get(columnName)
				...
				intialData.get(requiredRecords.get(iter)).put(columnName, String.valueOf(updatedAmount));
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
		}
