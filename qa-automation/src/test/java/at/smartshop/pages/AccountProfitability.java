...
public class AccountProfitability extends Factory {
	...
	private static final By TBL_ACCOUNT_PROFITABILITY = By.cssSelector("#rptdt");
	private static final By TBL_ACCOUNT_PROFITABILITY_GRID = By.cssSelector("#rptdt > tbody");
	...
	private static String grossSales;
	private static String taxes;
	private static String deposits;
	...
	public Map<Integer, Map<String, String>> getTblRecordsUI() {
		try {
			int recordCount = 0;
			tableHeaders.clear();
			WebElement tableReportsList = getDriver().findElement(TBL_ACCOUNT_PROFITABILITY_GRID);
			WebElement tableReports = getDriver().findElement(TBL_ACCOUNT_PROFITABILITY);
	...
	public void calculateAmount(String columnName, String price, String tax) {
		try {
			for (int iter = 0; iter < reportsData.size(); iter++) {
				...
				grossSales = String.valueOf(updatedAmount);
				intialData.get(iter).put(columnName, Constants.DOLLAR_SYMBOL + grossSales);
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
		}
	...
	public void calculateTax(String columnName, String tax) {
		try {
			for (int iter = 0; iter < reportsData.size(); iter++) {
				String initialAmount = intialData.get(iter).get(columnName).replaceAll(Reports.REPLACE_DOLLOR,
						Constants.EMPTY_STRING);
				double updatedAmount = Double
						...
						+ Double.parseDouble(initialAmount);
				updatedAmount = Math.round(updatedAmount * 100.0) / 100.0;
				taxes = String.valueOf(updatedAmount);
				intialData.get(iter).put(columnName, Constants.DOLLAR_SYMBOL + taxes);
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
		}
	...
	public void calculateDeposit(String columnName, String deposit) {
		try {
			for (int iter = 0; iter < reportsData.size(); iter++) {
				String initialAmount = intialData.get(iter).get(columnName).replaceAll(Reports.REPLACE_DOLLOR,
						Constants.EMPTY_STRING);
				double updatedAmount = Double
						.parseDouble(deposit.replaceAll(Reports.REPLACE_DOLLOR, Constants.EMPTY_STRING)) * 2
						+ Double.parseDouble(initialAmount);
				updatedAmount = Math.round(updatedAmount * 100.0) / 100.0;
				deposits = String.valueOf(updatedAmount);
				intialData.get(iter).put(columnName, Constants.DOLLAR_SYMBOL + deposits);
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
		}
	...
	public void calculateNetSales(String columnName) {
		try {
			for (int iter = 0; iter < reportsData.size(); iter++) {
				double updatedAmount = Double.parseDouble(grossSales) - Double.parseDouble(taxes)
						- Double.parseDouble(deposits);
				updatedAmount = Math.round(updatedAmount * 100.0) / 100.0;
