...
public class UFSReport extends Factory {
	...
	private static final By TBL_SALES_TIME_DETAILS = By.cssSelector("#table1");
...
//		foundation.threadWait(Constants.FIFTY_FIVE_SECONDS);
	...
	public void getTblRecordsUIOfSalesTimeDetails() {
		try {
			int recordCount = 0;
			...
			for (WebElement footerRowValue : footerRow) {
				for (int columnCount = 1; columnCount < tableHeadersOfSalesTimeDetails.size() + 1; columnCount++) {
					...
					tableFooterData.put(tableHeadersOfSalesTimeDetails.get(columnCount - 1), column.getText());
				}
			}
		} catch (Exception exc) {
			TestInfra.failWithScreenShot(exc.toString());
