...
//			// resetting test data
	...
	public void verifyKioskTransactionsWithRDS() {
		try {
			...
			CustomisedAssert.assertTrue(foundation.isDisplayed(Order.LBL_YOUR_ORDER));
			CustomisedAssert
					.assertTrue(foundation.isDisplayed(order.objText(rstV5DeviceData.get(CNV5Device.PRODUCT_NAME))));
...
//			foundation.click(LocationSummary.LBL_TAX_CAT_SAVE);
	...
	public void verifySubsidyBalanceForConsumer() {
		...
		try {
			...
			text = foundation.getText(AccountLogin.LBL_CONSUMER_NAME);
			CustomisedAssert.assertTrue(text.contains(requiredData.get(9)));
