...
public class V5Test extends TestInfra {
	...
	public void englishItalianLanguage() {
		try {
			...
			List<String> productSearchPage = Arrays
			...
			List<String> createAccountPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.CREATE_ACCOUNT).split(Constants.DELIMITER_TILD));
			...
			foundation.click(LandingPage.BTN_LOGIN);
			List<String> loginPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.LOGIN_PAGE).split(Constants.DELIMITER_TILD));
			...
			List<String> fundPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.FUND_ACCOUNT_PAGE).split(Constants.DELIMITER_TILD));
			...
			List<String> scanSetupPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.QUICK_SCAN_SETUP).split(Constants.DELIMITER_TILD));
			...
			List<String> fingerPrintPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.FINGER_PRINT_SETUP).split(Constants.DELIMITER_TILD));
			...
			List<String> changePinPageData = Arrays
					.asList(rstV5DeviceData.get(CNV5Device.CHANGE_PIN).split(Constants.DELIMITER_TILD));
	...
	@Test(description = "C142738 - SOS-24492 Verify alternate language is set to spanish in Kiosk when user set the Alternate Language as spanish and full sync is done in ADM")
	public void alternateSpanishLanguage() {
        ...
        foundation.click(LocationSummary.BTN_SYNC);
        foundation.click(LocationSummary.BTN_SAVE);
        foundation.waitforElement(LocationList.TXT_SPINNER_MSG, 5);
        login.logout();
        browser.close();
...
//		//Validating Create Account Page
		...
		foundation.click(createAccount.objText(accountEditPageData.get(6)));
			...
			//Boolean actualValueTimeOut = foundation.isDisplayed(createAccount.objText(timeOutPopupData.get(0)));
